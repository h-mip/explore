...
The full map's “Share map” control copies either a link to the current view or ready-to-paste iframe code. Map embeds are available at `/embed/ca/map/`, `/embed/es/map/` and `/embed/en/map/` under either site's base path. The hour, month, activity and place embeds are generated only when verified survey results are enabled. Embeds are marked `noindex`.
